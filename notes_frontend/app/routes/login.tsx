...
export async function loader({ request }: LoaderFunctionArgs) {
  /** If a user is already logged in, redirect to /app, otherwise render login. */
...
/**
 * PUBLIC_INTERFACE
 * Action accepts email/password and creates a session (demo auth).
 */
export async function action({ request }: ActionFunctionArgs) {
  /** Authenticates any non-empty email/password and logs the user in (demo only, no backend). */
    ...
    <div className="flex min-h-screen items-center justify-center bg-white">
      <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-8 shadow-sm">
        <div className="mb-6 text-center">
          ...
          <h1 className="text-xl font-semibold text-gray-900">Sign in to Notes</h1>
          <p className="mt-1 text-sm text-gray-600">Modern, minimal and fast.</p>
        </div>
